...
void Game::keyPressEvent(QKeyEvent *event){
    ...
    if(event->key() == Qt::Key_Left){
        ...
        if(players[0]->pos().x() > 800 *0/numPlayers+5){
            //qDebug() << "Moving left";
            players[0]->setPos(players[0]->pos().x()-5 ,players[0]->pos().y());
        }
    }
    if(event->key() == Qt::Key_Right){
        //qDebug() << "Right" << 800 *1/numPlayers-5;
        if(players[0]->pos().x() < 800 * 1/numPlayers-5){
            //qDebug() << "Moving right";
            players[0]->setPos(players[0]->pos().x()+5 ,players[0]->pos().y());
        }
    }
    if(event->key() == Qt::Key_Up){
        ...
        bullet->setPos(players[0]->pos().x(),players[0]->pos().y());
        scene->addItem(bullet);
    }
    if(event->key() == Qt::Key_A && numPlayers > 1){
        if(players[1]->pos().x() > 800 * 1/numPlayers + 5){
            players[1]->setPos(players[1]->pos().x()-5 ,players[1]->pos().y());
        }
    }
    if(event->key() == Qt::Key_D && numPlayers > 1){
        if(players[1]->pos().x() < 800 * 2/numPlayers - 5){
            players[1]->setPos(players[1]->pos().x()+5 ,players[1]->pos().y());
        }
    }
    if(event->key() == Qt::Key_W && numPlayers > 1){
        Bullet * bullet = new Bullet();
        bullet->setPos(players[1]->pos().x(),players[1]->pos().y());
        scene->addItem(bullet);
    }
    if(event->key() == Qt::Key_J && numPlayers > 2){
        if(players[2]->pos().x() > 800 * 2/numPlayers + 5){
            players[2]->setPos(players[2]->pos().x()-5 ,players[2]->pos().y());
        }
    }
    else if(event->key() == Qt::Key_L && numPlayers > 2){
        if(players[2]->pos().x() < 800 * 3/numPlayers -5){
            players[2]->setPos(players[2]->pos().x()+5 ,players[2]->pos().y());
        }
    }
    if(event->key() == Qt::Key_I && numPlayers > 2){
        Bullet * bullet = new Bullet();
        bullet->setPos(players[2]->pos().x(),players[2]->pos().y());
        scene->addItem(bullet);
    }
    if(event->key() == Qt::Key_Delete && numPlayers > 3){
        if(players[3]->pos().x() > 800 * 3/numPlayers + 5){
            players[3]->setPos(players[3]->pos().x()-5 ,players[3]->pos().y());
        }
    }
    if(event->key() == Qt::Key_PageDown && numPlayers > 3){
        if(players[3]->pos().x() < 800 * 4/numPlayers -5){
            players[3]->setPos(players[3]->pos().x()+5 ,players[3]->pos().y());
        }
    }
    if(event->key() == Qt::Key_Home && numPlayers > 3){
        Bullet * bullet = new Bullet();
        bullet->setPos(players[3]->pos().x(),players[3]->pos().y());
        scene->addItem(bullet);
    }
